...
def ReprojecByRegcover(S2TileGranule,SrcEPSG,TargetEPSG,ParaPath,ReprojectedImgPath):
    ...
    RepImg = np.zeros(shape=(Heigth,Width,3), dtype=np.int16)
    ...
    np.put(a=RepImg[:,:,0],ind=Ind,v=raster[0,:,:][PairX !=Fill_value])
    np.put(a=RepImg[:,:,1], ind=Ind, v=raster[1,:, :][PairX != Fill_value])
    np.put(a=RepImg[:,:,2], ind=Ind, v=raster[2, :, :][PairX != Fill_value])
    del Ind
    ...
    np.put(RepImg[:,:,0], EpValidRC, raster[0,:,:][PairXGapFiller != Fill_value])
    np.put(RepImg[:,:,1], EpValidRC, raster[1, :, :][PairXGapFiller != Fill_value])
    np.put(RepImg[:,:,2], EpValidRC, raster[2, :, :][PairXGapFiller != Fill_value])
    ...
    write_Img(RepImg, Path, target_srs, geotrans, Width, Heigth, im_bands=3, dtype=gdal.GDT_Float32)
    t9 = time.time()
    print('time8', t9 - t8)
...
ImgPath = r'D:\Composite\T49RGP_20191025T030811_TCI_10m.jp2'
ReprojectedImgPath = r'D:\Composite'
SrcEPSG,TargetEPSG = 32649,32650
ReprojecByRegcover(ImgPath,SrcEPSG,TargetEPSG,PairPath,ReprojectedImgPath)
