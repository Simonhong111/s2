...
def ParameterGenerator(S2TileGranule,SrcEPSG,TargetEPSG,ReprojectedImgPath,ParOutPath):
    ...
    Xpath = os.path.join(ParOutPath,'TestRepX{}GapFiller.tif'.format(pixel_spacing))
    Ypath = os.path.join(ParOutPath,'TestRepY{}GapFiller.tif'.format(pixel_spacing))
    write_Img(ParaMeterXGapFiller, Xpath, src_srs, geo_t,x_size,y_size,im_bands=1, dtype=gdal.GDT_UInt32)
    write_Img(ParaMeterYGapFiller, Ypath, src_srs, geo_t, x_size, y_size, im_bands=1, dtype=gdal.GDT_UInt32)
...
repimgpath = r'C:\Users\zmhwh\Desktop\Temp\T50RKU_20190105T030111_B04_20mRep66.tif'
ParameterGenerator(ImgPath,32650,32649,repimgpath,r'C:\Users\zmhwh\Desktop\Temp')
