...
def fileload(filename,White_Reflectance = 0.73):
    ...
    White1 = table.row_values(2)
    White2 = table.row_values(3)
    White3 = table.row_values(4)
    White4 = table.row_values(5)
    White5 = table.row_values(6)

    for row in range(table.nrows)[7:]:
    ...
    mWhite1 = (np.array(White1[30:]).astype(np.float) - mDark)/White_Reflectance
    mWhite2 = (np.array(White2[30:]).astype(np.float) - mDark) / White_Reflectance
    mWhite3 = (np.array(White3[30:]).astype(np.float) - mDark) / White_Reflectance
    mWhite4 = (np.array(White4[30:]).astype(np.float) - mDark) / White_Reflectance
    mWhite5 = (np.array(White5[30:]).astype(np.float) - mDark) / White_Reflectance
    mWhite = (mWhite1 + mWhite2 + mWhite3 + mWhite4 + mWhite5)/5.0

    # temp = mWhite1 + mWhite2 + mWhite3 + mWhite4 + mWhite5
    # temp2 = np.array([mWhite1,mWhite2, mWhite3, mWhite4,mWhite5])
    #
    # temp2min = temp2.min(axis=0)
    # print("**",temp2min)
    # temp2max = temp2.max(axis=0)
    # temp2maxmean = np.abs(temp2max - temp2.mean(axis=0))
    # temp2minmean = np.abs(temp2min - temp2.mean(axis=0))
    # mask = np.where(temp2maxmean > temp2minmean)
    # temp2max[mask] = temp2min[mask]
    # mWhite = (temp  - temp2min - temp2max)/3.0
...
def getRedNir(Title):
    ...
    for idx, Lmd in enumerate(HeadTitle):
        ...
        if any([wave in str(Lmd) for wave in ["645.1","645.2","645.3","645.4"]]):
            Red = str(Lmd)
            Red_idx = idx
            Red_Flag = True
...
def readATCParam(path_txt=r"D:\SIFProject\ATCParams.txt"):
    waveLen = []
    Trans = []
...
def plotRef(Data,HeadTitle,White,LeftStart =755,LeftEnd = 759,InStart=759,InEnd = 764,weight=1):
    ...
    extent = (HeadTitle > 720) & (HeadTitle < 800)
    ext = np.where(extent)
    wavelen,trans = readATCParam()
    waveext = (wavelen> 719) & (wavelen < 801)
...
#
# Data,HT,White =fileload(r"D:\HUBEIFIELDDATA\20180830airline1-5\15350964458442982835611128394267_20180830113249.06.xlsx",White_Reflectance = 0.73)
# print(len(Data))
# intr = 0
# endd = intr+1
