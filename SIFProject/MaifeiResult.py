...
def Resultload(resultFilePath):
    ...
    return mData
def sameday(Dataset,month,day,field):
    slope2 = []
    intercept2 = []
    rvalue2 = []
    pvalue2 = []
    ...
    for Data in Dataset:
        ...
        pvalue = []
        time = []
        if Data[1] == month and Data[2] == day and Data[0] == field:
            for s in range(12):
    ...
    # plt.close(fig4)
    plt.show()
def plotD(resultFilePath,month,day,field):
    Dataset = Resultload(resultFilePath)
    sameday(Dataset, month, day,field)
