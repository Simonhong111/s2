...
def GetFieldGeoInfo(croppath,res):
    """
    ...
    dataSource = driver.Open(croppath, 0)
    layer = dataSource.GetLayer()

    extent = layer.GetExtent() # LonMinimum LonMaximum LatMinimum LatMaximum
    print("Field Extent is ",extent)
    ...
    info["convexhull"] = convexhull
    # print(info)

    return info

# GetFieldGeoInfo(croppath,1)

def LL2Proj(srcsrs,dstsrs,lonlat):
    """
    :param srcsrs:原投影
    :param dstsrs: 目标投影
    :param lonlat: 经纬度
    ...
    ulx,uly = ct.TransformPoint(lonlat[1],lonlat[0])[:2] # 先纬度后经度
    return ulx,uly
...
def GenImgMatrix(croppath,spec_path,res,DIS=1):

    FieldGeoInfo = GetFieldGeoInfo(croppath,res)
...
croppath = r"D:\Satellive\downshp\downland.shp"

# croppath = r"D:\Satellive\uppershp\upperland.shp"
# spec_path = \
#     r"D:\Satellive\SimonHong_Code\HUBEIFIELDDATA\20180828airline1-5\15350964458442982835611128394267_20180824090644.14.xlsx"

spec_dir = r"D:\Satellive\SimonHong_Code\HUBEIFIELDDATA\20180918airline1-5"
import  glob
spec_fns = glob.glob(os.path.join(spec_dir,"*.xlsx"))

for spec_path in spec_fns:
    GenImgMatrix(croppath,spec_path,1)
