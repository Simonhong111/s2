...
def search(tile, level='1C', start='20150523', end=datetime.datetime.today().strftime('%Y%m%d'), maxcloud=100,
           minsize=25.):
...
def download(products_df, output_dir=os.getcwd()):
    ...
    if products_df.empty == True:
        ...
        raise Exception('WARNING: No products found to download. Check your search terms.')
        ...
        for uuid, filename in zip(products_df['uuid'], products_df['filename']):
            ...
            #     print
            #     'Skipping file %s, as it has already been downloaded in the directory %s. If you want to re-download it, delete it and run again.' % (
            #     filename, output_dir)
            if os.path.exists('%s/%s' % (output_dir, filename)):

                print('Skipping file {}, as it has already been downloaded in the directory {}. If you want to re-download it, delete it and run again.'.format(filename,output_dir))


            elif os.path.exists('%s/%s' % (output_dir, filename[:-5] + '.zip')):

                # print
                # 'Skipping file %s, as it has already been downloaded and extracted in the directory %s. If you want to re-download it, delete it and run again.' % (
                # filename, output_dir)
                print('Skipping file {}, as it has already been downloaded and extracted in the directory {}. If you want to re-download it, delete it and run again.'.
...
def decompress(zip_files, output_dir=os.getcwd(), remove=False):
    ...
    for zip_file in zip_files:
        ...
        if os.path.exists('%s/%s' % (output_dir, zip_file.split('/')[-1].replace('.zip', '.SAFE'))):
            ...
            print('Extracting %s'.format(zip_file))
...
def main(username, password, tiles, level='1C', start='20150523', end=datetime.datetime.today().strftime('%Y%m%d'),
         maxcloud=100, minsize=25., output_dir=os.getcwd(), remove=False):
    ...
    for tile in tiles:
        ...
        mend = time.time()
        print ("download this image cost {} s".format(mend-mstart))
        #解压数据，此处注释掉了
        # # Decompress data
...
#         description='Download Sentinel-2 data from the Copernicus Open Access Hub, specifying a particular tile, date ranges and degrees of cloud cover.')
#
...
#     required.add_argument('-t', '--tiles', type=str, required=True, nargs='*',
#                           help="Sentinel 2 tile name, in format ##XXX")
#
#     # Optional arguments
#     optional.add_argument('-l', '--level', type=str, default='1C',
#                           help="Set to search and download level '1C' (default) or '2A' data. Note that L2A data may not be available at all locations.")
#     optional.add_argument('-s', '--start', type=str, default='20150523',
#                           help="Start date for search in format YYYYMMDD. Defaults to 20150523.")
#     optional.add_argument('-e', '--end', type=str, default=datetime.datetime.today().strftime('%Y%m%d'),
#                           help="End date for search in format YYYYMMDD. Defaults to today's date.")
#     optional.add_argument('-c', '--cloud', type=int, default=100, metavar='%',
#                           help="Maximum percentage of cloud cover to download. Defaults to 100 %% (download all images, regardless of cloud cover).")
#     optional.add_argument('-m', '--minsize', type=int, default=25., metavar='MB',
#                           help="Minimum file size to download in MB. Defaults to 25 MB.")
#     optional.add_argument('-o', '--output_dir', type=str, metavar='PATH', default=os.getcwd(),
#                           help="Specify an output directory. Defaults to the present working directory.")
#     optional.add_argument('-r', '--remove', action='store_true', default=False,
#                           help="Remove level 1C .zip files after decompression.")
#
#     # Get arguments from command line
#     args = parser.parse_args()
#
#     # Run through entire processing sequence
#     main(args.user, args.password, args.tiles, level=args.level, start=args.start, end=args.end, maxcloud=args.cloud,
#          minsize=args.minsize, output_dir=args.output_dir, remove=args.remove)


main(username="",password="",tiles=["49RGP", "49RGQ", "50RKU", "50RKV","50RLV"],level="2A",start="20190101",end="20191231",maxcloud=35,minsize=100,output_dir=r"F:\Sen2data",remove=False)
# main(username="simonrs",password="simon2738",tiles=["50SKA", "49RCQ", "49RDQ", "49REQ"],level="1C",start="20180501",end="20181031",maxcloud=35,minsize=100,output_dir=r"F:\Sen2data",remove=False)
