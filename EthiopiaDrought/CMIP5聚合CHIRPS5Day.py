...
def agg(rfpath):
    ...
    crop_path = rfpath
    reference_path = r"D:\Cornell\EthiopianDrought\CMIPMonth\Big\cmip5_long_2006.tif"
    ...
    path = os.path.join(r"D:\Cornell\EthiopianDrought\0ExperimentData\Precipitation_Data\P_Pentad_Aggragation",'agg_'+os.path.basename(rfpath))
    print(path)
    write_Img(Aggarr, path, reference_proj, reference_geo,Width, Height,im_bands=1, dtype=gdal.GDT_Float32)

import glob

chirps = glob.glob(r"D:\Cornell\EthiopianDrought\Chirps5DAverage\chirps-v2.0.*.tif")
for chip in chirps:
    agg(chip)