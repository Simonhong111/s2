...
def monthComp(cmipclippeddirectory,outputdirectory,year):
    ...
    for dt in (rrule.rrule(rrule.MONTHLY, interval=1, dtstart=start, until=stop)):
        ...
        if os.path.exists(chirps_file):
            bandNum += 1
        assert os.path.exists(chirps_file),"this file does not exist: "+ chirps_file
    ...
    path = os.path.join(outputdirectory,"chirps-v2.0.Long_Rains_{}.tif".format(year))
    write_Img(compdata, path, proj, geotrans, Width, Height, im_bands=1, dtype=gdal.GDT_Float32)

for year in range(1981,2019):

    monthComp(r"D:\Cornell\EthiopianDrought\Chirps2", r"D:\Cornell\EthiopianDrought\CHIRPSRainSeasonCom\SeasonCom",
                  year)
