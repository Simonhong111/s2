...
def ExtractCSV(path,croptype,CropPolygon):
    ...
    data = pd.read_csv(os.path.join(path,croptype+".csv"))
    ...
    evimask = np.where(EVIValues == -3000)
    EVIValues = EVIValues * 0.0001
    EVIValues[evimask] = -9999

    gsifmask = np.where(GOSIFValues >= 32766)
    GOSIFValues = GOSIFValues * 0.0001
    GOSIFValues[gsifmask] = -9999

    nsifmask = np.where(NEWSIFValues < -0.05)

    NEWSIFValues[nsifmask] = -9999
    ...
    outpath = r"D:\Cornell\EthiopianDrought\CropCSV\PolyGonCase\{}_VegIndex.csv".format(croptype)
