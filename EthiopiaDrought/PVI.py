...
def write_Img(data, path, im_width, im_heigth,im_bands=1, dtype=gdal.GDT_Float32):
    ...
    rainfall = gdal.Open(r"D:\Cornell\EthiopianDrought\CHIRPSDailyAgg\Big\agg_chirps-v2.0.2003.01.01.tif")
...
def PVI(chirpsdirectory,year):
    ...
    for dt in (rrule.rrule(rrule.DAILY, interval=1, dtstart=start, until=stop)):
        ...
        chirps_file = os.path.join(chirpsdirectory,
                                   "agg_chirps-v2.0.{}.{}.{}.tif".format(str(dt.year),str(dt.month).zfill(2),str(dt.day).zfill(2)))
    ...
    # return
    path = r'D:\Cornell\EthiopianDrought\AData\PVIDaily\Big'
...
for i in range(2006,2019):
    PVI(r"D:\Cornell\EthiopianDrought\CHIRPSDailyAgg\Big", i)