...
def write_Img(data, path, im_width, im_heigth,im_bands=1, dtype=gdal.GDT_Float32):
    ...
    rainfall = gdal.Open(r"D:\Cornell\EthiopianDrought\0ExperimentData\Precipitation_Data\P_Pentad_Average\chirps-v2.0.2003.01.1.tif")
...
def PVI(chirpsdirectory,year,month):
    start = datetime.strptime("-".join([str(year), str(month).zfill(2), "01"]), "%Y-%m-%d").date()
    DayNum = monthrange(year,month)[1]
    stop = datetime.strptime("-".join([str(year), str(month).zfill(2), str(DayNum).zfill(2)]), "%Y-%m-%d").date()
    bandNum = 0
    axisTime = []
    ...
    # return
    path = r'D:\Cornell\EthiopianDrought\0ExperimentData\PVI_Data\PVI_4M_Month'

    path = os.path.join(path,"pvi_{}{}.tif".format(year,str(month).zfill(2)))
    pvi[mask>0] = -9999
...
for i in range(2003,2019):
    for m in range(1,13):
        PVI(r"D:\Cornell\EthiopianDrought\0ExperimentData\Precipitation_Data\P_Pentad_Average", i,m)