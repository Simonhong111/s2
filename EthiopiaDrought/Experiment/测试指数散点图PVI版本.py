...
# 生成有效范围
ref_path =r"D:\Cornell\EthiopianDrought\CropType2015\agg_clip50.tif"
...
for Year in Years:
    ...
    for m in range(1, 7):
        ...
        ax.set_ylabel("PVI")
        if m <= 3:
            ax.text(0.3, 0.2, "R = {}".format(round(r_value, 3)), fontsize=16, color="r")
            ax.text(0.3, 0.1, "P = {}".format(round(p_value, 3)), fontsize=16, color="r")
        else:

            ax.text(0.4, 0.3, "R = {}".format(round(r_value, 3)), fontsize=16, color="r")
            ax.text(0.4, 0.2, "P = {}".format(round(p_value, 3)), fontsize=16, color="r")

    fig.tight_layout()  # 调整整体空白
    path2 = os.path.join(r"D:\Cornell\EthiopianDrought\CropCSV\DailyCrop\5day\Image5west", DataType + Year + "NPVI50.jpg")
    plt.savefig(path2)
    plt.close()
