...
def write_Img(data, path, im_width, im_heigth,im_bands=1, dtype=gdal.GDT_Float32):
    ...
    rainfall = gdal.Open(r"D:\Cornell\EthiopianDrought\CMIP5Pentand\cmip5.2007.01.1.tif")
...
def PVI(cmip5directory,year):
    ...
    for dt in (rrule.rrule(rrule.MONTHLY, interval=1, dtstart=start, until=stop)):
        ...
        for day in range(1,7):
            chirps_file = os.path.join(cmip5directory,
                                       "cmip5.{}.{}.{}.tif".format(str(dt.year), str(dt.month).zfill(2),
                                                                         str(day)))
    ...
    # return
    path = r'D:\Cornell\EthiopianDrought\0ExperimentData\PVI_Data\PVI_4M_5D_CMIP'
...
for i in range(2007,2019):
    PVI(r"D:\Cornell\EthiopianDrought\CMIP5Pentand", i)