...
if __name__ == '__main__':
    ...
    proj = reference.GetProjection()
    Width, Height = reference.RasterXSize, reference.RasterYSize
    SIFDir =r"D:\Cornell\EthiopianDrought\0ExperimentData\SIF_Data\SIF_V3_Ethiopia"
    OutDir = r"D:\Cornell\EthiopianDrought\0ExperimentData\SIF_Data\Detrend_SIF_V3_Ethiopia"
    SeasonType = "03"


    for SeasonType in ["02","03","04","05","06","07","08","09"]:
        bandNum = 0

        for year in range(st_year, 2019):

            sif_file = os.path.join(SIFDir,
                                    "SIF005_{}{}.nc.tif".format(str(year), SeasonType))
        ...
        for year in range(st_year, 2019):
            ...
            sif_file = os.path.join(SIFDir,
                                    "SIF005_{}{}.nc.tif".format(year, SeasonType))
        ...
        for i in range(bandNum):
            deResults[:, :, i][maskarr == -9999] = -9999
            outputpath = os.path.join(OutDir, "SIF005_{}{}.nc.tif".format(i + st_year, SeasonType))
            write_Img(deResults[:, :, i], outputpath, proj, geotrans, Width, Height, im_bands=1, dtype=gdal.GDT_Float32)
