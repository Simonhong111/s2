...
plt.title("{}  Map ".format(yy) + '\n', fontsize=16)


ramn = chirpsMap(rp, rpp, yy, str(mm))
ax = fig.add_subplot(2, 3, 1)
ax.set_title("Precipitaion_" + mm +" rains")
...
cax = ax.imshow(ramn, cmap=cmap, vmin=vmin, vmax=vmax)
...
samn = NewSIFMap(sp, spp, yy, str(mm))
ax1 = fig.add_subplot(2, 3, 2)
ax1.set_title("New SIF_" + mm +" rains")
mask = np.where(samn > -9999)
samn[samn == -9999] = np.nan
vmin = samn[mask].min()
vmax = samn[mask].max()
print("maxmin value", vmin, vmax)
cax1 = ax1.imshow(samn, cmap=plt.get_cmap("RdBu"), vmin=vmin, vmax=vmax)
...
eamn = eviMap(ep, epp, yy, str(mm))
ax2 = fig.add_subplot(2, 3, 3)
ax2.set_title("EVI_"  + mm +" rains")
mask = np.where(eamn > -9999)
eamn[eamn == -9999] = np.nan
vmin = eamn[mask].min()
vmax = eamn[mask].max()
print("maxmin value",vmin,vmax)
cax2 = ax2.imshow(eamn, cmap=plt.get_cmap("RdBu"), vmin=vmin, vmax=vmax)
cbar2 = plt.colorbar(cax2,ax=ax2,fraction=0.036,pad=0.04)
ax2.set_xticks([])
ax2.set_yticks([])

eamn = GOSIFMap(gsif, gsifp, yy, str(mm))
ax3 = fig.add_subplot(2, 3, 4)
ax3.set_title("GOSIF_" + mm +" rains")
mask = np.where(eamn > -9999)
eamn[eamn == -9999] = np.nan
vmin = eamn[mask].min()
vmax = eamn[mask].max()
print("maxmin value",vmin,vmax)
cax3 = ax3.imshow(eamn, cmap=plt.get_cmap("RdBu"), vmin=vmin, vmax=vmax)
cbar3 = plt.colorbar(cax3,ax=ax3,fraction=0.036,pad=0.04)
ax3.set_xticks([])
ax3.set_yticks([])

eamn = PviMap(pvip, pvipp, yy,pvitype=mm)
ax4 = fig.add_subplot(2,3,5)
ax4.set_title("PVI_"+mm +" rains")
mask = np.where(eamn > -9999)
eamn[eamn == -9999] = np.nan
vmin = eamn[mask].min()
vmax = eamn[mask].max()
print("maxmin value",vmin,vmax)
cax4 = ax4.imshow(eamn, cmap=plt.get_cmap("RdBu"), vmin=vmin, vmax=vmax)
