...
def monthComp(cmipclippeddirectory,outputdirectory,year,month):
    refpath = r"D:\Cornell\EthiopianDrought\CHIRPSDailyAgg\Big\agg_chirps-v2.0.2003.01.01.tif"
    ...
    print(compdata[3,2])
    path = os.path.join(outputdirectory,"chirps-v2.0.long_{}.tif".format(year))
    write_Img(compdata, path, proj, geotrans, 13, 20, im_bands=1, dtype=gdal.GDT_Float32)

for year in range(2006,2019):
    for month in range(1,13):
        monthComp(r"D:\Cornell\EthiopianDrought\CHIRPSDailyAgg\Big", r"D:\Cornell\EthiopianDrought\ChirpsDailyMonth\Big",
                  year, month)
