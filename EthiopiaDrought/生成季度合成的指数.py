...
path = r"D:\Cornell\EthiopianDrought\CropCSV\Crop\PolyGonAgg.csv"
...
for id,Year in enumerate(range(2010,2017)):
...
evi_short = (evi_short - evi_short.mean(axis=1)[:,np.newaxis])/evi_short.std(axis=1)[:,np.newaxis]
evi_long = (evi_long - evi_long.mean(axis=1)[:,np.newaxis])/evi_long.std(axis=1)[:,np.newaxis]
#
rf_short = (rf_short - rf_short.mean(axis=1)[:,np.newaxis])/rf_short.std(axis=1)[:,np.newaxis]
rf_long = (rf_long - rf_long.mean(axis=1)[:,np.newaxis])/rf_long.std(axis=1)[:,np.newaxis]
#
spvi = (spvi - spvi.mean(axis=1)[:,np.newaxis])/spvi.std(axis=1)[:,np.newaxis]
lpvi= (lpvi - lpvi.mean(axis=1)[:,np.newaxis])/lpvi.std(axis=1)[:,np.newaxis]

nsif_short = (nsif_short - nsif_short.mean(axis=1)[:,np.newaxis])/nsif_short.std(axis=1)[:,np.newaxis]
nsif_long = (nsif_long - nsif_long.mean(axis=1)[:,np.newaxis])/nsif_long.std(axis=1)[:,np.newaxis]


gsif_short = (gsif_short - gsif_short.mean(axis=1)[:,np.newaxis])/gsif_short.std(axis=1)[:,np.newaxis]
gsif_long = (gsif_long - gsif_long.mean(axis=1)[:,np.newaxis])/gsif_long.std(axis=1)[:,np.newaxis]



rf_short_anom = rf_short < -1.0
rf_long_anom = rf_long < -1.0
...
DataDict = {}
YM = [str(ym) for ym in range(2010,2017)]
...
outpath = r"D:\Cornell\EthiopianDrought\CropCSV\Crop\PolyGonAgg_anom.csv"
