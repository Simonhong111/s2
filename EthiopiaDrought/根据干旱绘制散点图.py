...
for Year in range(2010,2017):
    ...
    for m in range(1, 7):
        ...
        if m == 1:
            ...
            ylabel = "short RF anom"
            xlabel = "short EVI anom"
            Var1List, Var2List, Freq = monthAnalysis(path, "Short", "EVI", "RF", Year)
            droughtID = list(set(list(Freq)))
        if m == 2:
            ylabel = "short RF anom"
            xlabel = "short GSIF anom"
            Var1List, Var2List, Freq = monthAnalysis(path, "Short", "GSIF", "RF", Year)
            droughtID = list(set(list(Freq)))
        if m == 3:
            ylabel = "short RF anom"
            xlabel = "short NSIF anom"
            Var1List, Var2List, Freq = monthAnalysis(path, "Short", "NSIF", "RF", Year)
            droughtID = list(set(list(Freq)))

        if m == 4:
            ylabel = "long RF anom:"
            xlabel = "long EVI anom"
            Var1List, Var2List, Freq = monthAnalysis(path, "Long", "EVI", "RF", Year)
            droughtID = list(set(list(Freq)))
        if m == 5:
            ylabel = "long RF anom"
            xlabel = "long GSIF Anomaly"
            Var1List, Var2List, Freq = monthAnalysis(path, "Long", "GSIF", "RF", Year)
            droughtID = list(set(list(Freq)))
        if m == 6:
            print(m)
            ylabel = "long RF anom"
            xlabel = "long NSIF anom"
            Var1List, Var2List, Freq =monthAnalysis(path, "Long", "NSIF", "RF", Year)
            droughtID = list(set(list(Freq)))
    ...
    ax.scatter([0.96, 0.96, 0.96, 0.98, 0.98,0.98], [1.5, 1, 0.5,1.5, 1, 0.5], c=['b','g','orange','brown','r','k'])
