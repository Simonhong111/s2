...
def ExtractRFCSV(path,Vartype,Monthtype,defval=-9999):
    ...
    with open(path, newline='') as csvfile:
        ...
        for row in reader:
            ...
            Points.append([float(row["CE"]),float(row["CN"])])
...
for m in range(1, 8):
    ...
    for freq in droughtID:
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(VarValue[:, 0][mask], CropValue[:, 0][mask])
...
path2 = os.path.join(r"D:\Cornell\EthiopianDrought\CropCSV\Crop",cropclass+mMonthType+Years[year]+".jpg")
plt.savefig(path2)
